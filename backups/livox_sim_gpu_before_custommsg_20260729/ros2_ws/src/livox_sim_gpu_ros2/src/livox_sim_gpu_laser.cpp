...
void GazeboRosLaser::OnScan(ConstLaserScanStampedPtr &_msg)
{
  // RCLCPP_INFO(rclcpp::get_logger("gpu_laser"), "OnScan called, pattern index: %zu/%zu", this->scan_pattern_index_, this->scan_pattern_.size());
    ...
    // Keep one output slot per scan-pattern sample so OpenMP cannot reorder points.
    // FAST_LIO's MID360 preprocessor uses point order and line id to infer
    // the relative time within a scan.
    struct Point3D {
        float x, y, z;
        float reflectivity;
        uint8_t tag;
    ...
    std::vector<Point3D> processed_points(points_to_process);
    ...
    #pragma omp parallel for reduction(+:invalid_range_count,out_of_bounds_count,invalid_distance_count)
    for (int i = 0; i < points_to_process; ++i) {
        ...
        processed_points[i].valid = false;
        ...
        if (range < this->parent_ray_sensor_->RangeMax() && std::isfinite(range)) {
            ...
            // MID360 scan pattern contains four interleaved laser lines.
            processed_points[i] = {
                static_cast<float>(x),
                static_cast<float>(y),
                static_cast<float>(z),
                0.0F,
                0U,
                static_cast<uint8_t>(current_index % 4U),
                true};
        } else {
            invalid_range_count++;
    ...
    const size_t valid_point_count = std::count_if(
        processed_points.begin(), processed_points.end(),
        [](const Point3D &point) { return point.valid; });
    cloud_msg.width = valid_point_count;
    ...
    if (valid_point_count > 0) {
        ...
        sensor_msgs::PointCloud2Iterator<float> iter_reflectivity(cloud_msg, "reflectivity");
        ...
        for (const auto& pt : processed_points) {
            if (!pt.valid) {
                continue;
            }
            *iter_x = pt.x;
            *iter_y = pt.y;
            *iter_z = pt.z;
            *iter_reflectivity = pt.reflectivity;
            *iter_tag = pt.tag;
            *iter_line = pt.line;
            ++iter_x;
            ++iter_y;
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("gpu_laser"), 
        ...
        valid_point_count, points_to_process, pattern_size);
